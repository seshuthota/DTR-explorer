...
# Upload trained artifacts to Hugging Face Hub from Kaggle.
# Example:
#   HF_TOKEN=hf_xxx HF_REPO_ID=username/dtr-tuned-1.2b-kaggle-lora \
#   bash scripts/kaggle/40_upload_hf.sh

LOCAL_DIR="${LOCAL_DIR:-models/dtr-tuned-1.2b-kaggle}"
HF_REPO_ID="${HF_REPO_ID:-}"
HF_REPO_TYPE="${HF_REPO_TYPE:-model}"
HF_PRIVATE="${HF_PRIVATE:-0}"
HF_REVISION="${HF_REVISION:-main}"
HF_COMMIT_MESSAGE="${HF_COMMIT_MESSAGE:-Upload trained artifact from Kaggle}"
HF_ALLOW_PATTERNS="${HF_ALLOW_PATTERNS:-}"
HF_IGNORE_PATTERNS="${HF_IGNORE_PATTERNS:-}"

if [[ -z "$HF_REPO_ID" ]]; then
  echo "HF_REPO_ID is required (example: username/dtr-tuned-1.2b-kaggle-lora)" >&2
  exit 1
fi
